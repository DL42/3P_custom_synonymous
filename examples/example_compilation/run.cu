...
void run_migration_equilibrium_simulation(GO_Fish::allele_trajectories & a, float migration_rate){
	...
	GO_Fish::run_sim(a,mutation,demography,migration,selection,inbreeding,dominance,dont_preserve,sample_strategy,a); //scenario simulation, start from migration equilibrium, sample both start and final generations
}
