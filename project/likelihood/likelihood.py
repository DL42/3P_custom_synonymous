...
ap_size = alpha_in.size
p_size = initial_p_array.size
alpha = my_expand_alpha(alpha_in)
print(alpha)
...
def re_param_neutral_alpha(x):
	...
	loglambda = -1*total_likelihood(obs_ref_sfs, obs_ref_sfs, theta_site, g_array, d_array, inbreeding, p_array, b_array, alpha, 0, mse)
	return loglambda
...
def re_param(x):
	...
	loglambda = -1*total_likelihood(obs_test_sfs, obs_ref_sfs, theta_site, g_array, d_array, inbreeding, p_array, boundary_array, alpha, 0, mse)
	return loglambda
...
def re_param_neu(x):
	...
	d_array = np.array([0.5],dtype=np.float32)
	b_array = np.array([0,0])
	loglambda = -1*total_likelihood(obs_test_sfs, obs_ref_sfs, theta_site, g_array, d_array, inbreeding, p_array, b_array, alpha, 0, mse)
	return loglambda
...
def re_param_neu_lethal(x):
	...
	loglambda = -1*total_likelihood(obs_test_sfs, obs_ref_sfs, theta_site, g_array, d_array, inbreeding, p_array, b_array, alpha, lethal_perc, mse)
	return loglambda
...
def re_param_lethal(x):
	...
	d_array = np.array([0.5]*g_array.size,dtype=np.float32)
	lethal_perc = x[x.size-1]
	loglambda = -1*total_likelihood(obs_test_sfs, obs_ref_sfs, theta_site, g_array, d_array, inbreeding, p_array, boundary_array, alpha, lethal_perc, mse)
	return loglambda
